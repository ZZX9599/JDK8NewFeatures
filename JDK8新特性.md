...
- 为了省去定义一个 Runnable 实现类的麻烦，不得不使用匿名内部类 必须覆盖重写抽象 run 方法
...
匿名内部类`在编译的时候会生成一个class文件`

Lambda在程序`运行的时候形成一个类 `
...
所以`函数式接口就是可以适用于Lambda使用的接口`。只有确保接口中有且仅有一个抽象方法
...
一旦用该注解来定义接口，编译器将会强制检查该接口是否确实有且仅有一个抽象方法，否则将会报错，不过，即使不使用该注解，只要满足函数式接口的定义，这仍然是一个函数式接口，使用起来都一样，只是多了一个检查
...
翻译为`供应商`，它意味着"供给" , 对应的Lambda表达式需要“对外提供”一个符合泛型类型的对象数据
...
public class TestSupplier {
    public static void main(String[] args) {
        printMax(()->{
            int[] ints = new int[]{1, 5, 2, 3, 7, 9, 12, 4, 644,76};
            Arrays.sort(ints);
...
public class TestConsumer {
    public static void main(String[] args) {
        conversion("JXLzzx",(str)->{
            System.out.println(str.toUpperCase());
            System.out.println(str.toLowerCase());
        });
    }

    public static void conversion(String str,Consumer<String> consumer){
        consumer.accept(str);
    }
}
```
...
根据一个类型的数据得到另一个类型的数据，前者称为前置条件， 后者称为后置条件。有参数有返回值
...
public class TestFunction {
    ...
    public static void strConversion(Function<String,Integer> function){
...
行加工 处理。Stream可以看作是流水线上的一个工序。在流水线上，通过多个工序让一个原材料加工成一
...
注意：除了终结方法外，其余方法均为非终结方法

![image-20221008102219127](.\assets\image-20221008102219127.png)
...
如果需要将Stream中的`Integer类型数据转成int类型`，可以使用 mapToInt 方法
